...
const Main = () => {
  ...
  return (
    <div className='bgContainer w-full min-h-screen'>
      <div className='w-fit max-w-[90%] px-4 py-10 mx-auto flex flex-col gap-y-6'>

        {questions && questions.map((question, index) => {
          return <QuestionGroup key={question.id} id={question.id} question={question.question} answers={question.answers} selectedAnswer={selectedAnswers[index].answer} setSelectedAnswers={setSelectedAnswers} finalData={isCompleted ? correctAnswers[index] : null} />
        })}
        ...
        <div className='text-center mt-2'>
          {isCompleted ?
            <div className='flex justify-center items-center gap-x-6 gap-y-2 flex-wrap'>
              ...
              <button onClick={handleReset} className='bg-btnPrimary text-sm text-btnText rounded-2xl px-10 py-2.5'>Play again</button>
            </div>
